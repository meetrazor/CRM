...
if(isset($_GET['filter']['vendor']) && $_GET['filter']['vendor'] != '')
{
    $sql_where .= " AND (cm.vendor_id in (" . implode(',', $_GET['filter']['vendor'])."))";
}

if(isset($_GET['filter']['created_by']) && $_GET['filter']['created_by'] != '')
{
    $sql_where .= " AND (cm.user_id in (" . implode(',', $_GET['filter']['created_by'])."))";
}
