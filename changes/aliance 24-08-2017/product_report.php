...
$condition = '1=1';
$selectedRm = array();
$misCondition = 'am.activity_on = "prospect" and am.disposition_id is not null';
...
$joinTable = array(
    ...
    array("left","campaign_category AS cc","cc.campaign_id = cm.campaign_id",array("ctm.category_name")),
    array("left","category_master AS ctm","ctm.category_id = cc.category_id")
);
$misDisRes = $db->JoinFetch($mainTable,$joinTable,$misCondition,array("ctm.category_name"=>"asc"),null,"dm.disposition_id,ctm.category_id");
$misData = $db->FetchToArrayFromResultset($misDisRes);
$dispositionFormatData = array();

foreach($misData as $key => $value){
    $dispositionFormatData[$value['category_name']][$value['disposition_name']] = $value['number'];
}
    ...
    <div class='row-fluid' id="print">
        <div class="span12" id="display_mis">
            <table class="table" id="dg_mis">
                ...
                <?php
                if(count($dispositionFormatData) > 0){
                    foreach($dispositionFormatData as $campaign => $misData){
                        if($campaign != ''){
                            ?>
                            <tr>
