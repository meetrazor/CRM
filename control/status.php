...
//remove is time calculated becuase no time calculated fileds in database
    ...
    //     'is_show_to_kc','is_email_send','is_sms_send','is_time_calculate','null as lead_count'
    // );
        $columns = array('sm.status_id','sm.status_name','sm.status_type','sm.activity_type','sm.sort_order','sm.is_default','sm.is_close','sm.cal_price','sm.is_active','sm.is_callback',
...
// 	echo $sql;exit;
    ...
    if(is_array($idArray)){
        foreach($idArray as $typeId){
            $existingCheckR = $db->FetchToArray("lead_master","$table_id","status_id = '{$typeId}'");
            $existingCheckT = $db->FetchToArray("activity_master","$table_id","status_id = '{$typeId}'");
            // addOn $existingCheckInTickets
            $existingCheckInTickets = $db->FetchToArray("tickets","$table_id","status_id = '{$typeId}'");

            $existingCheckC = count($existingCheckR);
            $existingCheckCount = count($existingCheckT);
            // addOn $existingCheckTicketsCount
            $existingCheckTicketsCount = count($existingCheckInTickets);
            if($existingCheckC > 0 || $existingCheckCount > 0 || $existingCheckTicketsCount > 0){
