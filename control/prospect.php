...
// 	echo $sql;exit;
    ...
    while ($row = $db->MySqlFetchRow($main_query))
    {
        ...
        $row['last_call'] = ($row['last_call'] != '0000-00-00 00:00:00' && $row['last_call'] != '') ? core::YMDToDMY($row['last_call'],true) : "";
        $row['prospect_name'] = core::StripAllSlashes($row['prospect_name']);
        $response['aaData'][] = $row;
    }
    ...
    if(is_array($idArray)) {
        foreach ($idArray as $typeId) {
            $deleteA = $db->FetchRowWhere("lead_master", $table_id,"$table_id = '{$typeId}'");
            $deleteAB = $db->FetchRowWhere("activity_master", "activity_id","type_id = '{$typeId}' and source_type= 'prospect'");

            $existingCheckC = $db->CountResultRows($deleteA);
            $existingCheckCount = $db->CountResultRows($deleteAB);
    ...
    $html = '';
    $mcq = (isset($prospectDetail['mcq']) && $prospectDetail['mcq'] == 0)  ? "NO" : "YES";
    $gst = (isset($prospectDetail['gst']) && $prospectDetail['gst'] == 0)  ? "NO" : "YES";
    $itr = (isset($prospectDetail['itr']) && $prospectDetail['itr'] == 0)  ? "NO" : "YES";
    $bankStatement = (isset($prospectDetail['bank_statement']) && $prospectDetail['bank_statement'] == 0)  ? "NO" : "YES";
    $matches = (isset($prospectDetail['matches']) && $prospectDetail['matches'] == 0)  ? "NO" : "YES";
    $payment = (isset($prospectDetail['payment']) && $prospectDetail['payment'] == 0)  ? "NO" : "YES";
    $inPrincipleLetter = (isset($prospectDetail['in_principle_letter']) && $prospectDetail['in_principle_letter'] == 0)  ? "NO" : "YES";
    $directorCibil = (isset($prospectDetail['director_cibil']) && $prospectDetail['director_cibil'] == 0)  ? "NO" : "YES";
    $oneForm = (isset($prospectDetail['one_form']) && $prospectDetail['one_form'] == 0)  ? "NO" : "YES";
    if($prospect_id != ''){
        ...
        $html .= '<tr><th>ITR</th>';
        $html .= '<td>'.$itr.'</td><tr>';
        $html .= '<tr><th>Bank Statement</th>';
        $html .= '<td>'.$bankStatement.'</td><tr>';
        $html .= '<tr><th>Director Detail</th>';
        $html .= '<td>'.$directorCibil.'</td><tr>';
        $html .= '<tr><th>One Form</th>';
        $html .= '<td>'.$oneForm.'</td><tr>';
        $html .= '<tr><th>Matches</th>';
        $html .= '<td>'.$matches.'</td><tr>';
        $html .= '<tr><th>Payment Page</th>';
        $html .= '<td>'.$payment.'</td><tr>';
        $html .= '<tr><th>In Principle Letter</th>';
        $html .= '<td>'.$inPrincipleLetter.'</td><tr>';
    ...
    if($prospectId != ''){
        ...
        $joinTable = array(
            ...
            array("left","category_master as catm","catm.category_id = lm.category_id",array("catm.category_name")),
            array("left","vendor_master as vm","cm.vendor_id = vm.vendor_id",array("vm.vendor_name")),
            array("left","admin_user as au","au.user_id = lm.created_by",array("au.first_name as created_by")),
        ...
        if(count($prospectInfo) > 0){
            ...
            $html .= '<td>'.core::StripAllSlashes($prospectInfo[0]['first_name']." ".$prospectInfo[0]['last_name']).'</td><tr>';
            ...
            $mcq = (isset($prospectInfo[0]['mcq']) && $prospectInfo[0]['mcq'] == 0)  ? "NO" : "YES";
            $gst = (isset($prospectInfo[0]['gst']) && $prospectInfo[0]['gst'] == 0)  ? "NO" : "YES";
            $itr = (isset($prospectInfo[0]['itr']) && $prospectInfo[0]['itr'] == 0)  ? "NO" : "YES";
            $bankStatement = (isset($prospectInfo[0]['bank_statement']) && $prospectInfo[0]['bank_statement'] == 0)  ? "NO" : "YES";
            $matches = (isset($prospectInfo[0]['matches']) && $prospectInfo[0]['matches'] == 0)  ? "NO" : "YES";
            $payment = (isset($prospectInfo[0]['payment']) && $prospectInfo[0]['payment'] == 0)  ? "NO" : "YES";
            $inPrincipleLetter = (isset($prospectInfo[0]['in_principle_letter']) && $prospectInfo[0]['in_principle_letter'] == 0)  ? "NO" : "YES";
            $directorCibil = (isset($prospectInfo[0]['director_cibil']) && $prospectInfo[0]['director_cibil'] == 0)  ? "NO" : "YES";
            $oneForm = (isset($prospectInfo[0]['one_form']) && $prospectInfo[0]['one_form'] == 0)  ? "NO" : "YES";
            ...
            $html .= '<td>'.$prospectInfo[0]['category_name'].'</td><tr>';
            $html .= '<tr><th>Amount</th>';
            $html .= '<td>'.$prospectInfo[0]['amount'].'</td><tr>';
            $html .= '<tr><th>Actual Amount</th>';
            $html .= '<td>'.$prospectInfo[0]['actual_amount'].'</td><tr>';
            $html .= '<tr><th>Email</th>';
            ...
            $html .= '<tr><th>Prospect Via</th>';
            $html .= '<td>'.$prospectInfo[0]['prospect_via'].'</td><tr>';
            $html .= '<tr><th>Created On</th>';
            $html .= '<td>'.core::YMDToDMY($prospectInfo[0]['created_at'],true).'</td><tr>';
            $html .= '<tr><th>Created By</th>';
            $html .= '<td>'.$prospectInfo[0]['created_by'].'</td><tr>';
            $html .= '<tr><th>Vendor Name</th>';
            $html .= '<td>'.$prospectInfo[0]['vendor_name'].'</td><tr>';
            $html .= '<tr><th>MCQ</th>';
            $html .= '<td>'.$mcq.'</td><tr>';
            $html .= '<tr><th>GST</th>';
            $html .= '<td>'.$gst.'</td><tr>';
