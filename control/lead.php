...
//if(core::isAjax() && isset($_GET['etoken']) && $token == $_GET['etoken']) {
    if('fetch' == $action){
        ...
        $columns = array(
            'l.lead_id','l.lead_code','l.lead_name','null as campaign_name','l.email','l.remarks','am.activity_type','sm.status_name',
            'l.mobile_no','cam.category_name','l.amount','NULL as cw_status_name',
            ...
            'cm.customer_name',
        'l.created_at','concat(cu.first_name," ",cu.last_name) as created_by','l.updated_at','l.customer_check','l.check_time','s.state_name','c.city_name','l.pincode','l.prospect_id',
            'cmm.campaign_name'
    ...
    $seach_columns = array(
        ...
        $joins .= " LEFT JOIN admin_user as cu ON cu.user_id = l.created_by";
        ...
        $joins .= " LEFT JOIN prospect_master as p ON (p.prospect_id = l.prospect_id)";
        $joins .= " LEFT JOIN campaign_master as cmm ON (cmm.campaign_id = p.campaign_id)";
        $sql_where = "WHERE 1=1 and lu.is_latest = 1 and l.lead_id = 3962";

    if($leadId != ''){
        $sql_where .= " and l.lead_id  = '{$leadId}'";
    }
    if($statusId != ''){
        $sql_where .= " and l.status_id  = '{$statusId}'";
    }
      ...
      //  echo $sql;exit;
        ...
        {
            //$row['campaign_name'] = $db->FetchCellValue("campaign_master","campaign_name","campaign_id = (select campaign_id from prospect_master where prospect_id = '{$row['prospect_id']}')");
    ...
    //        $leadDetails .= "</td></tr>";
        ...
        if($leadId != ''){
            ...
            $joinTable = array(
                ...
                array("left","partner_master as pm","pm.partner_id = lm.partner_id",array('concat(pm.first_name," ",pm.last_name) as partner_name')),
                array("left","customer_master as cm","cm.customer_id = lm.customer_id",array("cm.customer_name")),
                array("left","prospect_master as prom","prom.prospect_id = lm.prospect_id",array("concat(prom.first_name,' ',prom.last_name) as prospect_name")),
                array("left","campaign_master as cam","cam.campaign_id = prom.campaign_id",array("cam.campaign_name")),
                array("left","campaign_type_master as ctm","ctm.campaign_type_id = cam.campaign_type_id",array("ctm.campaign_type_name")),
                array("left","category_master as catm","catm.category_id = prom.category_id",array("catm.category_name")),
                array("left","admin_user as bu","bu.user_id = lm.bd_id",array("concat(bu.first_name,' ',bu.last_name) as bd_name")),
                array("left","admin_user as cu","cu.user_id = lm.created_by",array("concat(cu.first_name,' ',cu.last_name) as created_by")),
            );
            $leadR = $db->JoinFetch($mainTable,$joinTable,"lm.lead_id in ($leadId)");
            $leadInfo = $db->FetchToArrayFromResultset($leadR);
            // get lead bd activity
            ...
            $activityHistoryQ = $db->JoinFetch($mainTable,$joinTable,"am.type_id = '{$leadId}' and am.source_type = 'lead'",array("am.created_at"=>"desc"));
            ...
            $activityHtml = '<div class="row-fluid">
                <div class="span11 table-responsive">
                    <table class="table">
                        ...
                        <tbody>';
                        foreach($activityHistory as $key => $historyData){
                            $lastActivityTime = ($key == 0) ? $historyData['created_at'] : $lastActivityTime;
                            $activityHtmlTr .= '<tr>';
                                $activityHtmlTr .= '<td>'.$historyData['remarks'].'</td>';
                                $activityHtmlTr .= '<td>'.$historyData['status_name'].'</td>';
                                $activityHtmlTr .= '<td>'.core::YMDToDMY($historyData['created_at'],true).'</td>';
                                $activityHtmlTr .= '<tr>';
                        }
                $activityHtml .= $activityHtmlTr;
                $activityHtml .= '</tbody>
                    </table>
                </div>
            </div>';
            ...
            if(count($leadInfo) > 0){
                $datetime1 = new DateTime($lastActivityTime);
                $datetime2 = new DateTime($leadInfo[0]['created_at']);
                $difference = $datetime1->diff($datetime2);
                $leadActivity = $activityHtml;
                ...
                $html .= '<tr><th>Loan Amount</th>';
                $html .= '<td>'.$leadInfo[0]['amount'].'</td><tr>';
                $html .= '<tr><th>Actual Amount</th>';
                $html .= '<td>'.$leadInfo[0]['amount_actual'].'</td><tr>';
                $html .= '<tr><th>Loan Product Type</th>';
                $html .= '<td>'.$leadInfo[0]['category_name'].'</td><tr>';
                ...
                $html .= '<tr><th>Campaign Type</th>';
                $html .= '<td>'.$leadInfo[0]['campaign_type_name'].'</td><tr>';
                $html .= '<tr><th>Campaign</th>';
                $html .= '<td>'.$leadInfo[0]['campaign_name'].'</td><tr>';
                $html .= '<tr><th>Activity</th>';
                $html .= '<td>'.$activityHtml.'</td><tr>';
                $html .= '<tr><th>BD Name</th>';
                $html .= '<td>'.$leadInfo[0]['bd_name'].'</td><tr>';
                $html .= '<tr><th>Lead Assign On</th>';
                $html .= '<td>'.core::YMDToDMY($leadInfo[0]['created_at']).'</td><tr>';
                //$html .= '<tr><th>Last Update Date(BD)</th>';
                //$html .= '<td>'.$leadActivity.'</td><tr>';
                $html .= '<tr><th>Remarks</th>';
                $html .= '<td>'.$leadInfo[0]['remarks'].'</td><tr>';
                $html .= '<tr><th>TeleCaller Name</th>';
                $html .= '<td>'.$leadInfo[0]['created_by'].'</td><tr>';
                $html .= '<tr><th>Time Period (No of days) From lead to current date</th>';
                $html .= '<td>'.$difference->format('%a').'</td><tr>';
