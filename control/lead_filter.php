...
{
    $sql_where .= " AND (l.created_by in (" . implode(',', $_GET['filter']['created_by'])."))";
}
if(isset($_GET['filter']['bd_id']) && $_GET['filter']['bd_id'] != '')
{
    $sql_where .= " AND (bu.user_id in (" . implode(',', $_GET['filter']['bd_id'])."))";
}
//echo $sql_where;
...
{
    $sql_where .= " AND (cmm.campaign_id in (" . implode(',', $_GET['filter']['campaign'])."))";
}
