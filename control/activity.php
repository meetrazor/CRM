...
if('fetch' === $action){
    ...
    $columns = array(
        ...
        'am.contact','catm.category_name','pm.amount',
        's.state_name','c.city_name', 'pm.pincode','dm.disposition_name',
        ...
        'am.duration','dm.is_close','dm.is_callback','pm.prospect_id','am.is_latest',"dm.is_meeting"
 ...
 //   $joins .= " left join prospect_contact as pc on (pc.prospect_id = pm.prospect_id)";
    ...
    // filtering
// filtering
    $sql_where = "WHERE 1=1 and am.disposition_id is not null and am.source_type = 'prospect' and ".$activityCondition;
    ...
    if($userId != ''){
        $sql_where .= " and am.created_by = '{$userId}'";
    }
    ...
    {
        $seach_condition = "";
        $seach_condition .= 'contact' . " LIKE '%" . mysql_real_escape_string( $_GET['sSearch_6'] ) . "%' OR ";
        $seach_condition = '(' . substr($seach_condition, 0, -3) . ')';
...
// 	echo $sql;exit;
    ...
    {
        $row['prospect_created_at'] = ($row['prospect_created_at'] != '0000-00-00 00:00:00' && $row['prospect_created_at'] != '') ? core::YMDToDMY($row['prospect_created_at'],true) : "";
        $row['start_date_time'] = ($row['start_date_time'] != '0000-00-00 00:00:00' && $row['start_date_time'] != '') ? core::YMDToDMY($row['start_date_time'],true) : "";
        $row['remarks'] = ($row['remarks'] != '') ? core::StripAllSlashes($row['remarks']) : "";
        $response['aaData'][] = $row;
    }
