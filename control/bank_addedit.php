...
if('addedit' === $action){
    ...
    $validator = array(
        ...
        'messages' => array(
            'bank_name' => array('required' => "Please enter bank name"),
        )
    );
    ...
    if(count($errors) > 0){
        ...
        $id = (isset($data['bank_id']) && $data['bank_id'] != '') ? $data['bank_id'] : 0;


        $data['is_active'] = isset($data['is_active']) ? 1 : 0;

        $response = array();
        $exist_condition = "bank_name='{$data['bank_name']}'";

        if($id > 0){

            $exist_condition .= " && bank_id != '$id'";
