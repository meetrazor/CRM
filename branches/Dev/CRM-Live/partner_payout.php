...
$(function() {
    ...
    dg_partner_payout = $('#dg_partner_payout').dataTable({
        ...
        "sAjaxSource": "control/partner_payout.php",//change here
        ...
        "aoColumns": [
            {
                mData: "partner_payout_id",
                bSortable : false,
                mRender: function (v, t, o) {
                    return '<label><input type="checkbox" id="chk_'+v+'" name="partner_payout_id[]" value="'+v+'"/><span class="lbl"></span></label>';
                },
                sClass: 'center'
            },
            { "mData": "partner_name" },
            ...
            {"mData": "remarks"},
            {
                ...
                mRender: function(v,t,o){
                    ...
                    <?php
                    if(($acl->IsAllowed($login_id,'PARTNER', 'Payout', 'Edit Partner Payout')))   {
                    ?>
                    act_html = act_html + "<a href='javascript:void(0);' onclick=\"EditRecord('"+ o['partner_payout_id'] +"')\" class='btn btn-minier btn-warning' data-placement='buttom' data-rel='tooltip' data-original-title='Edit' title='Edit'><i class='icon-edit bigger-120'></i> </a>&nbsp";
                    <?php } ?>

                    <?php
                    if(($acl->IsAllowed($login_id,'PARTNER', 'Payout', 'Delete Partner Payout')))   {
                    ?>
                    act_html = act_html + "<a href='javascript:void(0);' onclick=\"DeleteRecord('"+ o['partner_payout_id'] +"')\" class='btn btn-minier btn-danger' data-placement='bottom' data-rel='tooltip' data-original-title='Delete' title='Delete'><i class='icon-trash bigger-120'></i></a> ";
                    <?php } ?>
                    return act_html;
                }
            }
    ...
    $('#add_partner_payout').click(function(){
    ...
    $('#edit_partner_payout').click( function (e) {

        var selected_list = $('#dg_partner_payout tbody input[type=checkbox]:checked');
        ...
        //console.log(ele);
        var aData = dg_partner_payout.fnGetData( ele );

        $.each(aData, function(key,val){
            var inputType = $('form#frm_partner_payout #'+key).prop("type");
            if(inputType == 'checkbox'){
                if(val == 1){
                    $('form#frm_partner_payout #'+key).prop("checked",true);
                } else {
                    $('form#frm_partner_payout #'+key).prop("checked",false);
                }
            }else {
                if($('form#frm_partner_payout #'+key).length){
                    $('form#frm_partner_payout #'+key).val(val);
                }
            }
        });
        $('#act_partner_payout').text('Edit');
        $('form#frm_partner_payout').find('div.control-group').removeClass("success error");
        $('form#frm_partner_payout').find('div.control-group span').text('');

        $('#action').val('edit');
        $('#partner_payout_code').prop('readonly', true);
        $('#modal_add_partner_payout').modal('show');
    });


    $('#delete_partner_payout').click(function(){

        var delete_ele = $('#dg_partner_payout tbody input[type=checkbox]:checked');
        ...
        if(0 == selected_length){
            ...
            bootbox.confirm("Are you sure to delete selected record(s)?", function(result) {
                if(result) {
                    ...
                    $.ajax({
                        url: 'control/partner_payout.php?act=delete',
                        type:'post',
                        dataType:'json',
                        data:{ id : delete_id, },
                        success: function(resp){
                            dg_partner_payout.fnStandingRedraw();
                            if(resp.success) {
...
function EditRecord(id){
    ...
    $('#edit_partner_payout').click();
}

function DeleteRecord(fid){
    $('#chk_'+fid).prop('checked', true);
    $('#delete_partner_payout').click();
}
</script>
        ...
        <div class="row-fluid">
            <div class='span12'>
                <div class="table-header">
                    <?php echo $partnerName; ?> Payout List
					<span class="widget-toolbar pull-right">
                        <?php
                        if(($acl->IsAllowed($login_id,'PARTNER', 'Payout', 'Add Partner Payout')))   {
                            ?>
                            <a id='add_partner_payout' href="javascript:void(0);" data-placement="top" data-rel="tooltip" data-original-title="Add" class="white"><i class="icon-plus icon-large white"></i>Add</a>&nbsp|
                        <?php } ?>

                        <?php
                        if(($acl->IsAllowed($login_id,'PARTNER', 'Payout', 'Edit Partner Payout')))   {
                            ?>
                            <a id='edit_partner_payout' href="javascript:void(0);" data-placement="top" data-rel="tooltip" data-original-title="Edit" class="white"><i class="icon-pencil icon-large white"></i>Edit</a>&nbsp|
                        <?php } ?>

                        <?php
                        if(($acl->IsAllowed($login_id,'PARTNER', 'Payout', 'Delete Partner Payout')))   {
                            ?>
                            <a id='delete_partner_payout' href="javascript:void(0);" data-placement="top" data-rel="tooltip" data-original-title="Delete" class="white"><i class="icon-trash icon-large white"></i>Delete</a>
                        <?php } ?>
					</span>
                </div>
                <table id='dg_partner_payout' class="table table-condensed table-bordered table-hover">
                    <thead>
                    <tr>
