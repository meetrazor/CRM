...
if($action == 'edit'){
    ...
    // $data['is_time_calculate'] = (isset($data['is_time_calculate'])) ? 1 : 0;
    ...
    $status_res = $db->FetchRowWhere($table, array('status_name'),"status_name='$statusName' and status_type = '".$data['status_type']."' and status_id != '{$statusId}'");
    ...
    if($status_count > 0){
        ...
        if($data['is_default'] == 1){
            $newConditionCheck = "is_default = '1' && status_type = '{$data['status_type']}' && status_id != '{$statusId}'";
        ...
        $flag = ($db->UpdateWhere($table, $data,"status_id = '{$statusId}'")) ? 1 : 0;
        if($flag == 1){
