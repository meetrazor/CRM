...
$table_id = "payment_type_id";
$response='';
if('fetch' == $action){
    $table = "payment_type_master";

    $columns = array(
        'payment_type_id','payment_type_name','payment_url','is_file_upload','is_active'
    );
    $seach_columns = array(
        'payment_type_name'
    );
    ...
    if(is_array($idArray)){
        foreach($idArray as $typeId){
            ...
            if($existingCheckC > 0){
                $response['success'] = false;
                $response['title'] = 'Error:';
                $response['msg'] = "You can't delete payment type because it has record";
                break;
            } else {
