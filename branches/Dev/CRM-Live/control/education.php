...
$table_id ="education_id";

$action = $db->FilterParameters($_GET['act']);
if('fetch' === $action){

    // the table being queried
    $table = "education_master";//"city ct";

    // the columns to be filtered, ordered and returned
    // must be in the same order as displayed in the table

    $columns = array('education_id','education_name','description','is_active','is_default');
    $seach_columns = array('education_name','description');
...
// 	echo $sql;exit;
    ...
    {
        $row['education_name'] = stripslashes($row['education_name']);
        $response['aaData'][] = $row;
    }
    ...
    $validator = array(
        ...
        'rules' => array(
            'education_name' => array('required' => true),
        ),
        'messages' => array(
            'education_name' => array('required' => 'Please enter education name'),
        )
    );
    ...
    if(count($errors) > 0){
        ...
        //$data['education_name'] = ucwords(strtolower($data['education_name']));
        $data['is_active'] = (isset($data['is_active'])) ? 1 : 0;
        $data['is_default'] = (isset($data['is_default'])) ? 1 : 0;

        $educationName = $data['education_name'];
        ...
        if(isset($_POST['action']) && $_POST['action'] != ''){
            ...
            if($_POST['action'] == 'add'){
                ...
                $education_res = $db->FetchRowWhere($table, array('education_name'),"education_name='$educationName'");
                $sourceEducationRes = $db->FetchRowWhere($table, array('education_name'),"is_default = 1");
                $sourceEducationCount = $db->CountResultRows($sourceEducationRes);
                $education_count = $db->CountResultRows($education_res);
                if($education_count > 0){
                    $response['success'] = false;
                    $response['title'] = 'Error:';
                    $response['msg'] = "Education is Already Exist";
                }elseif($sourceEducationCount == 1 && $sourceEducationCount == $data['is_default']){
                    $response['success'] = false;
                    $response['title'] = 'Error:';
                    $response['msg'] = "Default education is Already Exist";
                }else{
                    ...
                    $response['msg'] = "Education added successfully!";
                }
            }elseif($_POST['action'] == 'edit'){

                if($data['education_id'] != ''){
                    $education_res = $db->FetchRowWhere($table, array('education_name'),"education_name='$educationName' and education_id != '{$data['education_id']}'");
                    $education_count = $db->CountResultRows($education_res);
                    if($education_count > 0){
                        $response['success'] = false;
                        $response['title'] = 'Error:';
                        ...
                        $condition = "education_id='$educationId'";
                        $db->UpdateWhere($table,$data, $condition);
                        $response['success'] = true;
                        $response['title'] = 'Successful';
                        $response['msg'] = "education updated successfully!";
                    }

                }else{
                    $response['success'] = false;
                    $response['title'] = 'Failed';
                    $response['msg'] = "Invalid education";
                }
            }
        }else{
    ...
    if(is_array($idArray)){
        foreach($idArray as $typeId){
            $existingCheckR = $db->Fetch("partner_education",$table_id,"$table_id = '{$typeId}'");
            $existingCheckC = $db->CountResultRows($existingCheckR);
            if($existingCheckC > 0){
                $response['success'] = false;
                $response['title'] = 'Error:';
                $response['msg'] = "You can't delete education because it has record";
                break;
            } else {
    ...
    $data['education_name'] = ucwords($_POST['term']);

    $exist_count = $db->FunctionFetch($table, 'count', $table_id,"education_name='{$data['education_name']}'");

    if($exist_count == 0){
        $data['description'] = $data['education_name'];
        ...
        $response['value'] = $row['education_id'];
        $response['text'] = $row['education_name'];
        echo json_encode($response);
    }
}
