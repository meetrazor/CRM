...
//echo $action;
$table = "break_violation_master";
$table_id = "break_violation_id";
$response='';
if('fetch' == $action){
    $table = "break_violation_master";

    $columns = array(
        'break_violation_id','break_violation_name','is_active'
    );
    $seach_columns = array(
        'break_violation_name'
    );
    ...
    $breakTypeName = $data['break_violation_name'];
    if(isset($_POST['action']) && $_POST['action'] != ''){

        if($_POST['action'] == 'add'){
            $break_violation_res = $db->FetchRowWhere($table, array('break_violation_name'),"break_violation_name='$breakTypeName'");
            $break_violation_count = $db->CountResultRows($break_violation_res);
            if($break_violation_count > 0){
                $response['success'] = false;
                $response['title'] = 'Error:';
                $response['msg'] = "Break violation is Already Exist";
            }else{
            ...
            if($data['break_violation_id'] != ''){
                $break_violation_res = $db->FetchRowWhere($table, array('break_violation_name'),"break_violation_name='$breakTypeName' and break_violation_id != '{$data['break_violation_id']}'");
                $break_violation_count = $db->CountResultRows($break_violation_res);
                if($break_violation_count > 0){
                    $response['success'] = false;
                    $response['title'] = 'Error:';
                    ...
                    $condition = "break_violation_id=".$data['break_violation_id']."";
