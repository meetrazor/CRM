...
$table_id = "partner_ledger_id";
$response='';
if('fetch' == $action){
    $table = "partner_ledger as pl";

    $columns = array(
        'pl.partner_ledger_id','concat(pm.first_name," ",pm.last_name) as partner_name','pl.created_at',
        'IF(pl.ledger_from = "lead",lm.lead_name,"withdrawal") as particular',
        'IF(pl.ledger_type = "D",pl.amount,0) as debit_amount','IF(pl.ledger_type = "C",pl.amount,0) as credit_amount',
        'pl.ledger_balance','lm.lead_id','pl.ledger_from'
    );
    $seach_columns = array(
        'concat(pm.first_name," ",pm.last_name)','pl.ledger_balance','pl.amount'
    );

    $joins = " LEFT JOIN partner_master as pm ON pm.partner_id = pl.partner_id";
    $joins .= " LEFT JOIN lead_master as lm ON lm.lead_id = pl.type_id";
...
//                $payment_type_count = $db->CountResultRows($payment_type_res);
                if(1 < 0){
                    ...
                    $condition = "$table_id=".$data['partner_ledger_id']."";
                    ...
                    $ledgerData = array(
                        ...
                        "amount" => $data['amount'],
                        "ledger_type" => 'C',
                        "type_id" => $data['partner_ledger_id'],
                        "ledger_from" => "payout",
                    );
                    $ledgerData = array_merge($ledgerData,$db->TimeStampAtCreate($user_id));
                    $ledgerCondition = "type_id=".$data['partner_ledger_id']." and ledger_from = 'payout'";
