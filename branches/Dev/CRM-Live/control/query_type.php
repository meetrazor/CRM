...
//echo $action;
$table = "query_type_master";
$table_id = "query_type_id";
$response='';
if('fetch' == $action){
    $table = "query_type_master";

    $columns = array(
        'query_type_id','query_type_name','query_color','is_active','is_default'
    );
    $seach_columns = array(
        'query_type_name'
    );
    ...
    $query_typeName = $data['query_type_name'];
    if(isset($_POST['action']) && $_POST['action'] != ''){

        if($_POST['action'] == 'add'){
            $query_type_res = $db->FetchRowWhere($table, array('query_type_name'),"query_type_name='$query_typeName'");
            $query_type_count = $db->CountResultRows($query_type_res);
            ...
            if($data['query_type_id'] != ''){
                $query_type_res = $db->FetchRowWhere($table, array('query_type_name'),"query_type_name='$query_typeName' and query_type_id != '{$data['query_type_id']}'");
                $query_type_count = $db->CountResultRows($query_type_res);
                if($query_type_count > 0){
                    $response['success'] = false;
                    $response['title'] = 'Error:';
                    ...
                    $condition = "query_type_id=".$data['query_type_id']."";
    ...
    if(is_array($idArray)){
        foreach($idArray as $typeId){
            ...
            if($existingCheckC > 0){
                $response['success'] = false;
                $response['title'] = 'Error:';
                $response['msg'] = "You can't delete query type because it has record";
                break;
            } else {
