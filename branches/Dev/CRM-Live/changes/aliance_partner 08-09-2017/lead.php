...
$telecallerDd = $db->CreateOptions("html","admin_user",array("user_id","concat(first_name,' ',last_name)"),null,array("concat(first_name,' ',last_name)"=>"asc"));
$partnerName = "";
if($partnerId != ''){
...
//        phone : 480
    ...
    dg_lead = $('#dg_lead').dataTable({
        ...
        "fnServerParams": function ( aoData ) { //send other data to server side
            ...
            aoData.push({ "name": "act", "value": "fetch" },{ "name": "lead_id", "value": "<?php echo $leadId; ?>" },{ "name": "status_id", "value": "<?php echo $statusId; ?>" });
        ...
        "aoColumns": [
            ...
            },
            {
                ...
                mRender: function(v,t,o){
                    ...
                    ?>
                    if(o['is_close'] != 1) {
                        act_html = act_html + "<a href='lead_form_addedit.php?id="+ o['lead_id'] +"&token=<?php echo $token; ?>' class='btn btn-minier btn-warning' data-placement='bottom' data-rel='tooltip' data-original-title='Edit "+o['lead_name']+"'><i class='icon-edit bigger-120'></i></a>&nbsp";
                    } else {
    ...
    $('#dg_lead tbody').on('click', 'td.details-control', function () {
        ...
        if ( row.child.isShown() ) {
            ...
            console.log(leadId);
            $.ajax({
                url: 'control/lead_form_addedit.php?act=commentview',
                type:'post',
                dataType:'html',
                beforeSend: function(){
                    row.child( wait ).show();
    ...
    if (jQuery().validate) {
        ...
        $("#frm_comment").validate({
            ...
            submitHandler : function(e) {
                ...
                $(e).ajaxSubmit({
                    url: 'control/lead_form_addedit.php?act=addcomment',
                    type:'post',
                    beforeSubmit: function (formData, jqForm, options) {
                        $(e).find('button').hide();
                    },
            ...
            <div class="widget-body">
                <div class="row-fluid">
                    <form id ="frm_filter" class="form-inline" name ="frm_filter">
                        ...
                        <div class="control-group inline">
                            <label class="control-label" for="city_id">Telecaller</label>
                            <div class="controls">
                                <select class="chzn-select-filter" data-placeholder="Select Telecaller" multiple name="filter[telecaller][]">
                                    <option></option>
                ...
                <div class="table-header">
					...
					<span class="widget-toolbar pull-right">
                        ...
                        if(($acl->IsAllowed($login_id,'LEAD', 'Lead', 'Add Lead')))   {
                            ?>
                            <a id='add_lead' href="lead_form_addedit.php?token=<?php echo $token; ?>" class="" data-placement="top" data-rel="tooltip"  data-original-title="Add"><i class="icon-plus icon-large white"></i></a>&nbsp|
                        <?php }  ?>
                        ...
                        if(($acl->IsAllowed($login_id,'LEAD', 'Lead', 'Edit Lead')))   {
                            ?>
                            <a id='edit_lead' href="lead_form_addedit.php?token=<?php echo $token; ?>" class="" data-placement="top" data-rel="tooltip" data-original-title="Edit"><i class="icon-pencil icon-large white"></i></a>&nbsp|
                        <?php }  ?>
