...
$misDisRes = $db->JoinFetch($mainTable,$joinTable,$misCondition,array("cm.campaign_name"=>"asc"),null,"dm.disposition_id,cm.campaign_id");
$misData = $db->FetchToArrayFromResultset($misDisRes);
$dispositionFormatData = array();

foreach($misData as $key => $value){
    $dispositionFormatData[$value['campaign_name']][$value['disposition_name']] = $value['number'];
}
