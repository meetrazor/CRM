...
if('fetch' === $action){
    ...
    $columns = array('dm.disposition_id','dm.disposition_name','dm.disposition_code','pdm.disposition_name as parent_disposition_name','dm.is_close',
        'dm.is_callback','dm.is_meeting','dm.is_active','dm.is_default','dm.parent_disposition_id','null as prospect_count');
    $seach_columns = array('dm.disposition_name','dm.disposition_code');
...
// 	echo $sql;exit;
    ...
    while ($row = $db->MySqlFetchRow($main_query))
    {
        ...
        $joinTable = array(
            array("left","activity_master as am","am.type_id = pm.prospect_id"),
        );
        $countDetails = $db->JoinFetch($mainTable,$joinTable,"disposition_id=$dispositionId and source_type = 'prospect'");
    ...
    if(count($errors) > 0){
        ...
        if(isset($_POST['action']) && $_POST['action'] != ''){
            ...
            if($_POST['action'] == 'add'){
                ...
                $disposition_res = $db->FetchRowWhere($table, array('disposition_name'),"disposition_name='$dispositionName'");
                ...
                if($data['disposition_id'] != ''){
                    $disposition_res = $db->FetchRowWhere($table, array('disposition_name'),"disposition_name='$dispositionName' and disposition_id != '{$data['disposition_id']}'");
                    $disposition_count = $db->CountResultRows($disposition_res);
