...
if('fetch' === $action){
    ...
    $columns = array(
        ...
        'r.role_name','concat(ru.first_name," ",ru.last_name) as reporting_to',
        'au.email','au.mobile_no',
        'au.updated_at',
        'concat(au2.first_name," ",au2.last_name) as rcd_updated_by',
        'au.is_active','null as activity_count'
...
// 	echo $sql;exit;
    ...
    while ($row = $db->MySqlFetchRow($main_query))
    {
        ...
        $userId=$row['user_id'];
        $row['activity_count'] = $db->FunctionFetch("activity_master", "count", "activity_id", "created_by=$userId");
        $response['aaData'][] = $row;
    }
