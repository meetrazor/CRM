...
<!-- 		<link rel="stylesheet" href="http://fonts.googleapis.com/css?family=Open+Sans:400,300" /> -->
	...
	<!--basic scripts-->
		...
		?>
               <!--  <script type="text/javascript" src="<?php echo ASSETS.'/js/chosen.js';?>"></script> -->
		<!--ace scripts-->
        ...
        $(document).on("change",".select-loan-type",function () {
            ...
            $.ajax({
                ...
                dataType: 'html',
                beforeSend: function(){
                    $('.select_product_type_loader').show();
                },
                complete: function(){
                    $('.select_product_type_loader').hide();
                },
                success: function(resp){
                    $('.select-product-type').html(resp);
                    $('.select-product-type').trigger("liszt:updated");
                    $('.select-query-stage').trigger("chosen:updated");
                }
            });
        });
        ...
        $(document).on("change",".select-product-type",function () {
            ...
            $.ajax({
                url: 'control/ticket_addedit.php?act=get_reason_dd',
                async : false,
                data : {
                    id : product_type_id,
                    selected : 0,
                ...
                dataType: 'html',
                beforeSend: function(){
                    $('.select_reason_type_loader').show();
                },
                complete: function(){
                    $('.select_reason_type_loader').hide();
                },
                success: function(resp){
                    $('.select-reason-type').html(resp);
                    $('.select-reason-type').trigger("liszt:updated");
                    $('.select-query-stage').trigger("chosen:updated");
                }
            });
        });

        $(document).on("change",".select-reason-type",function () {
            var reason_type_id = $(this).val();
            $.ajax({
                url: 'control/ticket_addedit.php?act=get_query_stage_dd',
                async : false,
                data : {
                    id : reason_type_id,
                    selected : 0,
                ...
                dataType: 'html',
                beforeSend: function(){
                    $('.select_query_stage_loader').show();
                },
                complete: function(){
                    $('.select_query_stage_loader').hide();
                },
                success: function(resp){
                    $('.select-query-stage').html(resp);
                    $('.select-query-stage').trigger("liszt:updated");
                    $('.select-query-stage').trigger("chosen:updated");
                }
            });
        });

        $(document).on("change",".select-query-stage",function () {
            var query_stage_id = $(this).val();
            $.ajax({
                url: 'control/ticket_addedit.php?act=get_sub_query_stage_dd',
                async : false,
                data : {
                    id : query_stage_id,
                    selected : 0,
                ...
                dataType: 'html',
                beforeSend: function(){
                    $('.select_sub_query_stage_loader').show();
                },
                complete: function(){
                    $('.select_sub_query_stage_loader').hide();
                },
                success: function(resp){
                    $('.select-sub-query-stage').html(resp);
                    $('.select-sub-query-stage').trigger("liszt:updated");
                    $('.select-query-stage').trigger("chosen:updated");
                }
            });
        });
